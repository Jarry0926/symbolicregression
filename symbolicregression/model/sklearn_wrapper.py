...
class SymbolicTransformerRegressor(BaseEstimator):
    ...
    def init_state(
        ...
        state = State(None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None)
        state.scaler = scaler
        state.scale_params = scale_params
        state.inputs = inputs
        state.inputs_ids = inputs_ids
        state.scaled_X = scaled_X
        state.Y = Y
    ...
    def get_policy(
        ...
        policy = self.model.get_policy(state)  ##Forward transformer: returns predicted functions
        return policy
    ...
    def apply_action(
        ...
        state = self.model.apply_action(state, idx=idx)  ##Forward transformer: returns predicted functions
        return state
    
    def is_solution(self, state):
        return self.model.is_solution(state)

    def prep_for_refinement(self, state, verbose=False):
        inputs = state.inputs
        scaled_X = state.scaled_X
        Y = state.Y
        n_datasets = len(inputs)
        inputs_ids = state.inputs_ids
        scaler = state.scaler
        scale_params = state.scale_params
        outputs = self.model.sample_candidates_from_state(state)
