...
class TransformerModel(nn.Module):
    ...
    def init_state(self, state, max_len=200, top_p=1.0, sample_temperature=None):
        #print("INNER init_state")
        src_enc = state.encoded
        src_len = state.x_len

        # input batch
        state.bs = len(src_len)
        assert src_enc.size(0) == state.bs
        ...
        # positions
        state.positions = src_len.new(state.max_len).long()
        state.positions = (
            torch.arange(state.max_len, out=state.positions).unsqueeze(1).expand(state.max_len, state.bs)
        )

        # current position / max lengths / length of generated sentences / unfinished sentences
        state.cur_len = 1
        state.gen_len = src_len.clone().fill_(1)
        state.unfinished_sents = src_len.clone().fill_(1)
    ...
    def get_policy(self, state, max_len=200, top_p=1.0, sample_temperature=None):
        ...
        tensor = self.forward(
            "fwd",
            x=state.generated[:state.cur_len],
            lengths=state.gen_len,
            positions=state.positions[:state.cur_len],
            causal=True,
            src_enc=state.encoded,
            src_len=state.x_len,
            use_cache=True,
        )
        assert tensor.size() == (state.cur_len, state.bs, self.dim)
    ...
    def cleanup(self, state, max_len=200):
        # sanity check
        assert (state.generated == self.eos_index).sum() == 2 * state.bs
        state.generated = state.generated.unsqueeze(-1).view(state.generated.shape[0], state.bs)
        # @NOTE: We can technically recover this because currently store the generated and len, 
        # @NOTE: Need to check when we need to call this

    def has_stop_token(self, state):
        # stop when there is a </s> in each sentence
        return state.unfinished_sents.max() == 0

    def apply_action(self, state, max_len=200, top_p=1.0, sample_temperature=None, idx = None):
        # print("INNER apply action")
        self.cache = deepcopy(state.cache)

        # NOTE: We don't need to generate scores through transformer because we pass action by idx manualy
        # compute word scores
        #tensor = self.forward(
        #    "fwd",
        #    x=state.generated[:state.cur_len],
        ...
        #    use_cache=True,
        #)
        #assert tensor.size() == (1, state.bs, self.dim)
        #tensor = tensor.data[-1, :, :].to(self.dtype)  # (bs, dim)  ##BE CAREFUL
        #scores = self.proj(tensor)  # (bs, n_words)
        #print(f"Scores: {scores.argmax()}")

        # select next words from user action
        # @NOTE CALL THIS ACTION
        next_words = torch.tensor([idx]).to("cuda")
        assert next_words.size() == (state.bs,)

        # update generations / lengths / finished sentences / current length
        state.generated[state.cur_len] = next_words * state.unfinished_sents + self.pad_index * (
            1 - state.unfinished_sents
        )
        state.gen_len.add_(state.unfinished_sents)
        state.unfinished_sents.mul_(next_words.ne(self.eos_index).long())
        state.cur_len = state.cur_len + 1

        state.cache = deepcopy(self.cache)
        return state

    def is_solution(self, state, max_len=200):
        # Has been wrapped up with <EOS>
        if self.has_stop_token(state): 
            return True
        if state.cur_len < max_len:
            return False
        # Wrap up with <EOS>
        else:
            state.generated[-1].masked_fill_(state.unfinished_sents.byte(), self.eos_index)
            return True
    ...
    def generate(
        ...
        if sample_temperature  is None:
            self.actions_taken = []
        print("Generating")
        #print(f"{src_enc.shape}, {src_len.shape}")
